...
8. **Accepted-only; app-auth yes, LeetCode-auth no.** We do not track failed attempts and do not touch the authenticated **LeetCode** `submissionList` endpoint or store users' LeetCode session cookies (that's out of scope — PRD §11). This is distinct from our own **Supabase Auth**, which is required (users sign in to see their data). "No session-cookie logic" means *LeetCode's* cookie, not our app session.
